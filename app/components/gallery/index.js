...
class Gallery extends Component {
    ...
    AddContact(){
        const {UserName,UserCompany,UserOffice,UserPhoneNumber,UserEmail}=this.state;
        ...
        return (
            <View style={styles.container}>
                ...
                <TextInput
                    style={styles.input}
                    placeholder='Office'
                    onChangeText={(useroffice) => { if (/^[a-zA-ZàáâäãåąčćęèéêëėįìíîïłńòóôöõøùúûüųūÿýżźñçčšžÀÁÂÄÃÅĄĆČĖĘÈÉÊËÌÍÎÏĮŁŃÒÓÔÖÕØÙÚÛÜŲŪŸÝŻŹÑßÇŒÆČŠŽ∂ð ,.'-]+$/u.test(useroffice) || useroffice === '') { this.onChangeText('UserName', useroffice) } }}
                    maxLength={30}>
                </TextInput>
